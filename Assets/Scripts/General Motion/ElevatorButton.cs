...
public class ElevatorButton : MonoBehaviour {
	...
	void FixedUpdate () {
		if ((travelling == false) && (playerTransform.position.y > 10f)) {
			elevatorTransform.position = new Vector3 (elevatorTransform.position.x, 15f, elevatorTransform.position.z);
			groundFloor = false;
		} else if ((travelling == false) && (playerTransform.position.y < 10f)){
	...
	void OnTriggerStay(Collider other){
		if ((other.gameObject.tag == "Player")&&(travelling == false)){
